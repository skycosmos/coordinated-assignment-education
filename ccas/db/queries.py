...
from ccas.db.rl_api_client import rl_insert, rl_select, rl_update
...
def fetch_paper_by_id(
    ...
    *,
    ...
    """Return one row from `papers` by primary key, or None if missing."""
    c = client or get_supabase()
    resp = c.table("papers").select(columns).eq("id", paper_id).limit(1).execute()
