...
def main() -> None:
    ...
    parser.add_argument(
        "--metadata-csv",
        type=Path,
        default=po / "paper_to_metadata.csv",
        help="Input CSV with columns paper_name, title, year, authors",
    )
    parser.add_argument(
        "--output-csv",
        type=Path,
        default=po / "paper_to_scholar_id.csv",
    ...
    if args.merge:
        ...
        if args.merged_output and not merge_path.is_absolute():
            merge_path = Path(__file__).resolve().parent / merge_path
