# ------------------------------------- # 
# Project:          CCAS
# Objective:        Extract Information From Texts Using OpenAI

# Creator:          Tianyu Zheng
...
def extract_info_from_text(client, cleaned_text):
    """
    Args:
        client (OpenAI): OpenAI API client initialized with the API key.
        cleaned_text (str): The cleaned text extracted from the PDF.
    Returns:
        dict: A dictionary containing extracted information from both models.
    """
    ...
    # Process basic features using GPT-4o-mini
    ...
    if len(lines_mini) < 6:  # Expecting at least 6 lines from GPT-4o-mini response
        raise ValueError("Unexpected response format from GPT-4o-mini model. (Rows < 6)")
    ...
    for i, key in enumerate(expected_keys_mini):
        ...
        results[key] = match.group(1).strip() if match else None  # Use matched content or None

    # Process advanced features using GPT-4o
    ...
    if len(lines) < 7:  # Expecting at least 7 lines from GPT-4o response
        raise ValueError("Unexpected response format from GPT-4o model. (Rows < 7)")

    expected_keys = ['relevance', 'reason_relevance', 'institutions','mechanisms', 'mechanisms_about', 'list_preferences', 'priority']
